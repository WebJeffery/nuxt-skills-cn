...
响应式 [Web Animations API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API)。
...
可以是关键帧对象数组、关键帧对象或 `ref`。有关更多详细信息,请参阅[关键帧格式](https://developer.mozilla.org/en-US/docs/Web/API/Web_Animations_API/Keyframe_Formats)。
...
第三个参数接受持续时间数字或选项对象,除了 [KeyframeAnimationOptions](https://developer.mozilla.org/en-US/docs/Web/API/Element/animate#parameters) 之外还有以下附加属性:
...
export interface UseAnimateOptions
  ...
  /**
   * 使用 `useAnimate` 时将自动运行 play
   *
   * @default true
   */
  immediate?: boolean
  /**
   * 是否将动画的结束样式状态提交到正在动画的元素
   * 通常,您应该与此一起使用 `fill` 选项。
   *
   * @default false
   */
