---
name: nuxt
description: Nuxt 全栈 Vue 框架，具有 SSR、自动导入和基于文件的路由。用于开发 Nuxt 应用、服务器路由、useFetch、中间件或混合渲染。
metadata:
  author: Anthony Fu
  version: "2026.1.28"
...
| 主题 | 描述 | 参考 |
|-------|-------------|-----------|
| Composables 自动导入 | Vue API、Nuxt composables、自定义 composables、工具 | [features-composables](references/features-composables.md) |
...
| 状态管理 | useState composable、SSR 友好的状态、Pinia 集成 | [features-state](references/features-state.md) |
...
| SSR 与水合 | 避免上下文泄漏、水合不匹配、composable 模式 | [best-practices-ssr](references/best-practices-ssr.md) |

## 高级

| 主题 | 描述 | 参考 |
|-------|-------------|-----------|
| 层 | 使用可重用的层扩展应用程序 | [advanced-layers](references/advanced-layers.md) |
| 生命周期钩子 | 构建时、运行时和服务器钩子 | [advanced-hooks](references/advanced-hooks.md) |
| 模块开发 | 使用 Nuxt Kit 创建可发布的 Nuxt 模块 | [advanced-module-authoring](references/advanced-module-authoring.md) |