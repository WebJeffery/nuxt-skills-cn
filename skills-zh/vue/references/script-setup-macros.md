...
`<script setup>` 是 Vue SFC 使用 Composition API 的推荐语法。它提供更好的运行时性能和 IDE 类型推断。
...
为 slot props 提供类型提示。在 Vue 3.3+ 中可用。
