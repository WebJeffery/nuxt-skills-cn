...
**影响: 低** - 对于不进入或离开 DOM 的元素的动画，使用由 Vue 响应式状态触发的 CSS 基于类的动画。这比 `<Transition>` 更简单，更适合反馈动画，如抖动、脉冲或高亮效果。
...
使用 `animationend` 事件而不是 `setTimeout` 以获得更清晰的代码：
...
## 参考
- [Vue.js 动画技术 - 基于类的动画](https://vuejs.org/guide/extras/animation.html#class-based-animations)
