...
环境 API 将多个运行时环境形式化，超越了传统的客户端/SSR 分割。

## 概念

Vite 6 之前：两个隐式环境（`client` 和 `ssr`）。

Vite 6+：根据需要配置任意数量的环境（浏览器、Node 服务器、边缘服务器等）。

## 基本配置

对于 SPA/MPA，无需任何更改——选项应用于隐式的 `client` 环境：
...
环境继承顶层配置。某些选项（如 `optimizeDeps`）默认仅应用于 `client`。
