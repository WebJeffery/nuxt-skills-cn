---
name: monorepo
description: 使用 pnpm workspaces、集中化别名和 Turborepo 的 monorepo 设置。用于创建或管理多包仓库。
---
...
为 monorepo 启用 ESLint 缓存以加快 linting 速度。
...
对于包含许多包或构建时间较长的 monorepo，使用 Turborepo 进行任务编排和缓存。

有关详细配置，请参阅专门的 Turborepo skill。

## 集中化别名

为了在 Vite、Nuxt、Vitest 配置中获得更好的 DX，在项目根目录创建集中化的 `alias.ts`:
