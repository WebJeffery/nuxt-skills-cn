---
title: 路由参数变化不触发生命周期钩子
impact: HIGH
impactDescription: 在使用不同参数的路由之间导航会重用组件实例，跳过 created/mounted 钩子并留下过期数据
type: gotcha
...
**影响：高** - 在使用相同组件的路由之间导航时（例如，`/users/1` 到 `/users/2`），Vue Router 为了性能会重用现有的组件实例。这意味着 `onMounted`、`created` 和其他生命周期钩子不会触发，你会留下来自前一个路由的过期数据。

## 任务清单

- [ ] 使用 `watch` 监听路由参数进行数据获取
- [ ] 或使用组件内 `onBeforeRouteUpdate` 守卫
- [ ] 或使用 `:key="route.params.id"` 强制重新创建（效率较低）
- [ ] 永远不要仅依赖 `onMounted` 来处理依赖于路由参数的数据
...
| `/users/1?tab=a` 到 `/users/1?tab=b` | 否 | 是 | 否（不同的监听） |
...
3. **使用带有 `immediate: true` 的 `watch`** - 覆盖初始加载和更新
4. **`onBeforeRouteUpdate` 是导航感知的** - 适合必须在视图更新之前加载的数据
5. **`:key="route.fullPath"` 是大锤** - 仅在必要时使用

## 参考
- [Vue Router 动态路由匹配](https://router.vuejs.org/guide/essentials/dynamic-matching.html#reacting-to-params-changes)
- [Vue School：响应参数变化](https://vueschool.io/lessons/reacting-to-param-changes)
